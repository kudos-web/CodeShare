...
/// </summary>
public partial class Custom_FormControls_ImageCropperTool_AdminControls_CustomBaseImageEditor : CMSUserControl
{
    #region "Variables"
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        hdnAllowResize.Value = QueryHelper.GetString("allowResize", "true");
        hdnCropHeight.Value = QueryHelper.GetString("cropHeight", "");
        hdnCropWidth.Value = QueryHelper.GetString("cropWidth", "");
        ...
        ScriptHelper.RegisterScriptFile(Page, ResolveUrl("~/Custom/FormControls/ImageCropperTool/AdminControls/CustomBaseImageEditor.js"));
        // Initialize scripts
        ...
        if (imageType != ImageHelper.ImageTypeEnum.None)
        {
            if (!LoadingFailed)
            {
                ...
                string url = URLHelper.ResolveUrl("~/Custom/FormControls/ImageCropperTool/AdminControls/CustomImageEditorInnerPage.aspx?editorguid=" + InstanceGUID + "&versionnumber=" + CurrentVersion + "&width=400");
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(hdnCropWidth.Value) && !string.IsNullOrEmpty(hdnCropHeight.Value))
                    {
                        url = URLHelper.AddParameterToUrl(url, "cropWidth", hdnCropWidth.Value);
                        url = URLHelper.AddParameterToUrl(url, "cropHeight", hdnCropHeight.Value);
                        url = URLHelper.AddParameterToUrl(url, "allowResize", hdnAllowResize.Value);
                    }
    ...
    private void ClearCropInfo()
    {
        ...
        txtCropX.Text = "0";
        txtCropY.Text = "0";
        txtCropWidth.Text = QueryHelper.GetString("cropWidth", "0");
        txtCropHeight.Text = QueryHelper.GetString("cropHeight", "0");
        chkCropLock.Checked = true;
    }
